...
try {
  ...
  await jsonDialog.getByRole("button", { name: "전체 보기" }).click();
  assert.match(await jsonDialog.getByRole("tree", { name: "저장 JSON 트리" }).textContent(), /layoutSnapshot/);
  await jsonDialog.getByRole("button", { name: "모두 접기" }).click();
  assert.equal(await jsonDialog.locator('.json-tree__toggle[aria-expanded="false"]').count() > 0, true);
  await jsonDialog.getByRole("button", { name: "모두 펼치기" }).click();
  await jsonDialog.getByRole("button", { name: "코드 보기" }).click();
  assert.match(await jsonDialog.locator("pre").textContent(), /"layoutSnapshot"/);
  assert.equal(await jsonDialog.getByRole("button", { name: "AI 선택 기준" }).count(), 1);
