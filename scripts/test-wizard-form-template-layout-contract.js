...
assert.match(adminLayoutManager, /admin\.templateLayout\.revision/);
assert.match(adminLayoutManager, /requestRevision/);
assert.match(admin, /id="section-preset-manager-target"/);
assert.match(admin, /id="section-preset-manager-target"[\s\S]*?<section class="section-library-manager">/);
assert.doesNotMatch(admin, /<teleport to="#section-preset-manager-target"/);
assert.doesNotMatch(admin, /Template Section 구성/);
assert.doesNotMatch(admin, /\+ Section Preset 연결/);
assert.match(admin, /@layout-saved="loadWizardFormTemplates\(\{ fresh: true \}\)"/);
assert.match(adminLayoutManager, /VisualEditorDialogHost/);
assert.match(adminLayoutManager, /템플릿 기본 레이아웃 편집/);
assert.match(adminLayoutManager, /promo-admin-layout-saved/);
...
assert.match(editor, /notifyAdminLayoutSaved/);
