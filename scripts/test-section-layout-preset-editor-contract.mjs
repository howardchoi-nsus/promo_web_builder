...
const workbench = fs.readFileSync(path.join(root, "admin-app/src/components/SectionLayoutDomWorkbench.vue"), "utf8");
...
assert.match(manager, /archetype/);
assert.match(manager, /headlineCapacity/);
assert.match(manager, /bodyCapacity/);
assert.match(manager, /visualEmphasis/);
assert.match(manager, /mediaSafeSide/);
assert.match(manager, /mobileStrategy/);
assert.match(manager, /ctaProminence/);
assert.match(manager, /contentComplexity/);
...
assert.match(manager, /저장 JSON/);
assert.match(manager, /JsonSnapshotDialog/);
...
assert.match(workbench, /Section Live Preview/);
assert.match(workbench, /DOM 구성 Workbench/);
assert.doesNotMatch(workbench, /iframe/);
assert.match(frame, /promo-section-layout-saved/);
assert.match(frame, /sectionLayoutPresetService\.editorUrl/);
assert.match(frame, /VisualEditorDialogHost/);
assert.match(frame, /save-target-label="선택한 레이아웃 프리셋"/);
...
assert.match(editor, /WorkspaceSplitter/);
assert.match(editor, /selectedFieldKey/);
assert.match(editor, /TextEditorControls/);
