...
const tokenSet = {
  ...
  versions: [{ id: versionId, version: 2, status: "draft" }],
...
try {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (url.pathname === "/api/design-token-set-version" && request.method() === "PUT") {
      savedTokens = request.postDataJSON().tokens;
      return fulfill({ ok: true, tokenSet: { ...tokenSet, tokenSetId: setId, id: versionId, status: "draft", values: savedTokens } });
    }
  ...
  await page.locator(".sticky-actions").getByRole("button", { name: "저장" }).click();
