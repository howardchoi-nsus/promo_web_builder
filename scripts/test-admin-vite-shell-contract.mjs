...
assert.match(html, /selectAdminTab\('component-generation'\)/);
assert.match(html, /<component-generation-workspace @notify="setStatus"><\/component-generation-workspace>/);
assert.match(html, /v-if="selectedItemComponent && selectedItemComponent\.status !== 'archived'"/);
assert.match(html, /rel="stylesheet" href="\/prototype\/admin-assets\/promo-web-builder\.css\?v=admin-vite-v11"/);
