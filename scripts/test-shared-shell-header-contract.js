...
assert.match(appShellCss, /transition:\s*grid-template-columns var\(--app-motion-normal\) ease/);
assert.match(appShellCss, /\.shell-sidebar__mode \[data-shell-sidebar-mode="max"\]\s*\{[\s\S]*?display:\s*none/);
assert.match(appShellCss, /\.shell-frame\.is-sidebar-minimized \.shell-sidebar__mode \[data-shell-sidebar-mode="min"\]\s*\{[\s\S]*?display:\s*none/);
assert.match(appShellCss, /\.shell-frame\.is-sidebar-minimized \.shell-sidebar__mode \[data-shell-sidebar-mode="max"\]\s*\{[\s\S]*?display:\s*inline-flex/);
assert.match(legacyShellCss, /@import url\("\.\/app-shell\.css\?v=app-shell-sidebar-v4"\)/);
