...
assert.match(textControls, /emit\("patch-style", patch\)/);
assert.match(textControls, /aria-label="폰트 굵기"/);
assert.doesNotMatch(textControls, /:disabled="!hasLineSelection"\s*aria-label="글자 크기 디자인 토큰"/);
...
assert.match(renderer, /data-auto-height-measure="true"/);
