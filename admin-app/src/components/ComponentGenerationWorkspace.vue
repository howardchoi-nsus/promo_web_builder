...
export default {
  ...
  data: () => ({
    ...
    run: null, proposals: [], proposal: null, selectedProposalId: "", busy: false, saving: false, applying: false, applyResults: [], error: "", viewport: "desktop", mobileReviewed: false, pollTimer: null,
  }),
  computed: {
  ...
  methods: {
    ...
    async selectSource(event) {
      ...
      this.sourceId = ""; this.sourceImageUrl = ""; this.run = null; this.proposals = []; this.proposal = null; this.selectedProposalId = ""; this.crop = { x: 0, y: 0, width: 1, height: 1 };
    ...
    clearSource() { this.releaseUrl(); this.file = null; this.imageDataUrl = ""; this.sourceName = ""; this.sourceSize = 0; this.sourceDimensions = null; this.sourceId = ""; this.sourceImageUrl = ""; this.run = null; this.proposals = []; this.proposal = null; this.selectedProposalId = ""; if (this.$refs.sourceInput) this.$refs.sourceInput.value = ""; localStorage.removeItem(RUN_STORAGE_KEY); },
    ...
    async analyze() {
      if (!this.canAnalyze) return; this.busy = true; this.error = ""; this.proposals = []; this.proposal = null;
      try {
    ...
    setRun(run) { this.run = run; this.proposals = run?.proposals || (run?.proposal ? [run.proposal] : []); const selected = this.proposals.find((item) => item.id === this.selectedProposalId) || this.proposals[0] || null; this.proposal = selected; this.selectedProposalId = selected?.id || ""; if (run?.id) localStorage.setItem(RUN_STORAGE_KEY, run.id); if (run?.source?.imageUrl) { this.sourceImageUrl = run.source.imageUrl; if (!this.sourceUrl) this.sourceUrl = run.source.imageUrl; } if (run?.componentIntent) this.componentIntent = run.componentIntent; if (run?.allowedSectionRoles) this.selectedRoles = run.allowedSectionRoles; if (run?.source?.cropSpec) this.crop = run.source.cropSpec; if (["queued", "analyzing", "validating"].includes(run?.status)) this.schedulePoll(run.id); },
    selectProposal(item) { this.selectedProposalId = item.id; this.proposal = item; },
    candidateRegionStyle(item) { const region = item?.componentDefinition?.sourceRegion || { x: 0, y: 0, width: 1, height: 1 }; return { left: `${(this.crop.x + region.x * this.crop.width) * 100}%`, top: `${(this.crop.y + region.y * this.crop.height) * 100}%`, width: `${region.width * this.crop.width * 100}%`, height: `${region.height * this.crop.height * 100}%` }; },
    ...
    async saveReview() {
      ...
      try { const response = await fetch(`/api/component-generation-proposals?id=${encodeURIComponent(this.proposal.id)}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ componentDefinition: this.proposal.componentDefinition, renderSpec: this.proposal.renderSpec }) }); const result = await response.json(); if (!response.ok && !result.proposal) throw new Error(result.message || "검증 저장에 실패했습니다."); this.proposal = { ...this.proposal, ...(result.proposal || {}) }; this.proposals = this.proposals.map((item) => item.id === this.proposal.id ? this.proposal : item); this.$emit("notify", this.proposal.validationResult?.ok ? "검토 내용과 검증 결과를 저장했습니다." : "검증 오류가 남아 있습니다."); }
      catch (error) { this.error = error.message; } finally { this.saving = false; }
    },
    async applySelected() {
      const proposalIds = this.proposals.filter((item) => item.componentDefinition?.selected !== false && !item.appliedAt && item.validationResult?.ok && item.componentDefinition?.creationMode !== "new-version").map((item) => item.id);
      if (!proposalIds.length || this.applying) return;
      this.applying = true; this.error = "";
      try { const response = await fetch("/api/component-generation-proposals-apply", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ proposalIds }) }); const result = await response.json(); this.applyResults = result.results || []; if (!response.ok && !result.succeeded) throw new Error(result.message || result.error || "컴포넌트 Draft 생성에 실패했습니다."); await this.restoreRun(this.run.id); this.$emit("notify", `${result.succeeded || 0}개 컴포넌트 Draft를 생성했습니다.`); }
...
<template>
  <section class="generation" aria-labelledby="generation-title">
    ...
    <div v-else class="result-stack">
      <section class="card candidate-panel"><div class="card-title"><div><span>Detected components</span><h3>{{ proposals.length }}개 컴포넌트 후보</h3><p>큰 이미지에서 분리된 후보를 선택해 각각의 DOM과 필드를 검토하세요.</p></div></div><div class="candidate-list"><div v-for="(item,index) in proposals" :key="item.id" class="candidate-row" :class="{ active: item.id === selectedProposalId }"><label><input v-model="item.componentDefinition.selected" type="checkbox" :disabled="Boolean(item.appliedAt)"><span>{{ item.appliedAt ? '생성됨' : '생성 선택' }}</span></label><button type="button" @click="selectProposal(item)"><span>{{ index + 1 }}</span><strong>{{ item.componentDefinition.name }}</strong><em>{{ Math.round((item.confidence || 0) * 100) }}%</em></button></div></div></section>
      ...
      <p v-if="error" class="error">{{ error }}</p><ul v-if="applyResults.length" class="field-list"><li v-for="item in applyResults" :key="item.proposalId"><strong>{{ item.name || item.proposalId }}</strong><span>{{ item.ok ? 'Draft 생성 완료' : item.message }}</span></li></ul><div class="result-actions"><button class="tiny-button" type="button" @click="analyze">같은 이미지로 다시 분석</button><button class="tiny-button" type="button" @click="resetForNew">새 이미지 분석</button><button class="tiny-button" type="button" :disabled="saving" @click="saveReview">{{ saving ? '저장 중' : '현재 후보 재검증·저장' }}</button><button class="tiny-button primary" type="button" :disabled="applying || !mobileReviewed || !proposals.some((item) => item.componentDefinition?.selected !== false && !item.appliedAt && item.validationResult?.ok && item.componentDefinition?.creationMode !== 'new-version')" @click="applySelected">{{ applying ? 'Draft 생성 중' : '선택 후보 컴포넌트 Draft 생성' }}</button></div><label class="mobile-check"><input v-model="mobileReviewed" type="checkbox"><span>Mobile 미리보기와 오버플로를 확인했습니다.</span></label><p class="next-note">검증을 통과한 새 컴포넌트 후보만 일괄 Draft로 생성됩니다. 기존 컴포넌트의 새 버전 후보는 충돌 방지를 위해 개별 검토가 필요합니다.</p>
    </div>
  </section>
</template>
...
.candidate-list{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:7px}.candidate-list button{display:grid;grid-template-columns:24px 1fr auto;align-items:center;gap:7px;padding:10px;border:1px solid var(--line);background:var(--surface-2);color:var(--ink);text-align:left}.candidate-list button.active{border-color:var(--accent);background:var(--accent-soft)}.candidate-list button span,.candidate-box span{display:grid;place-items:center;width:22px;height:22px;background:var(--accent);color:#fff;font-size:11px;font-weight:900}.candidate-list button em{color:var(--sub);font-size:11px;font-style:normal}.candidate-box{position:absolute;padding:0;border:2px solid #f0b34c;background:rgb(240 179 76/.12);cursor:pointer}.candidate-box.active{z-index:2;border-color:#57d1ff;background:rgb(87 209 255/.2);box-shadow:0 0 0 2px rgb(87 209 255/.25)}.candidate-box span{position:absolute;top:2px;left:2px}
.candidate-row{display:grid;border:1px solid var(--line);background:var(--surface-2)}.candidate-row.active{border-color:var(--accent);background:var(--accent-soft)}.candidate-row>label{display:flex;align-items:center;gap:6px;padding:7px 10px;border-bottom:1px solid var(--line);font-size:11px}.candidate-row>button{border:0;background:transparent}.candidate-row.active>button{background:transparent}
</style>
