...
2. 섹션에 이미지 항목이 있으면 생성 이미지를 해당 이미지 항목에 적용한다.
3. 이미지 항목이 없으면 생성 이미지를 선택한 섹션의 배경 이미지로 적용한다.
...
- 이미지 항목 존재 여부에 따라 적용 대상을 결정적으로 선택한다.
...
선택한 섹션
  ├─ 사용 가능한 이미지 항목 존재
  │    └─ targetType=item, targetItemKey 지정
  └─ 사용 가능한 이미지 항목 없음
...
1. 관리자 설정으로 지정된 이미지 대상 항목
2. 잠금되지 않은 첫 번째 표시 가능 이미지 항목
3. 섹션 배경 이미지

잠금된 이미지 항목은 AI 적용 대상으로 선택하지 않는다. 섹션 배경 속성이 관리자에 의해 잠긴 경우 생성 버튼을 비활성화하고 사유를 표시한다.
...
- 이미지 항목: `<img>`에 생성 이미지 URL과 alt 적용
...
- 이미지 항목 대상이면 layout에서 이미지 item 위치와 텍스트 safe area를 계산한다.
- 섹션 배경 대상이면 배경 위 텍스트 가독성을 고려한 safe area와 overlay 권고를 생성한다.
- Gemini 생성 이미지는 Blob에 저장하고 내부 프록시 URL을 결과로 반환한다.

### 7.3 Apply API

- 현재 콘텐츠 hash, template version, layout revision을 재검증한다.
- `target.type=item`이면 이미지 항목의 `value`, `source`, `alt`만 갱신한다.
...
3. 이미지 항목이 없을 때 `section-background`를 선택한다.
...
- 이미지 항목이 있는 섹션은 `target.type=item`이 된다.
- 이미지 항목이 없는 섹션은 `target.type=section-background`가 된다.
- 잠긴 이미지 항목을 대상에서 제외한다.
...
4. 적용 후 Hero 이미지 항목에 이미지가 표시되는지 확인한다.
...
- 이미지 항목이 있는 섹션은 해당 항목에 이미지가 적용된다.
- 이미지 항목이 없는 섹션은 해당 섹션 배경에 이미지가 적용된다.
