...
## 6. 현행화 메모 (2026-07-23, 이미지 프롬프트 계약 갱신)

이 문서의 이미지 프롬프트는 이후 논의로 다음이 확정되어, 실제 적용은 `계획/ai-section-auto-design-development-plan-2026-07-23.md`(v2)의 계약을 따른다.

- **배경은 종속(subordinate)**: 헤드라인·CTA 뒤에 깔리므로 주제가 지배적이지 않게, safe zone(콘텐츠 자리)은 차분·저디테일로.
- **페이드를 이미지에 굽지 않는다**: 페이드·그라데이션·비네트·마스크는 이미지에 넣지 않고 **Renderer가 섹션 배경색 기준 CSS로 처리**한다(배경색/페이드 변경 시 재생성 불필요).
- **레이아웃 인지**: 컴포넌트 배치의 여집합을 배경 focal zone으로, 콘텐츠 자리를 safe zone으로 지정한다(`CompositionSpec`).
- **StyleProfile 연계**: 브랜드 룩(팔레트·재질·조명)은 선택된 디자인 토큰 세트에서 컴파일한 `StyleProfile`로 주입하고, 브랜드명·HEX를 시스템 프롬프트에 하드코딩하지 않는다.
- 여기 §3의 프롬프트는 "브랜드 톤 참조용"이며, 배포 계약 프롬프트(영/한)는 v2 계획서 부록 A·B를 정본으로 한다.
