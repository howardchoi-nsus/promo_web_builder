...
const PROMPT_TYPES = {
  ...
  promo_overview_parser: {
    ...
    body: [
      "Convert the user's promotion request into the supplied structured overview contract.",
      ...
      "Do not invent URLs, dates, legal terms, markets, benefits, or user-supplied facts.",
      "Keep an existing confirmed value when the new request does not clearly replace it.",
      "Return missing and uncertain fields explicitly.",
      "Natural-language request: {{naturalLanguage}}",
      "Current confirmed overview: {{currentOverviewJson}}",
      "Allowed values: {{allowedValuesJson}}",
    ].join("\n"),
    requiredVariables: ["naturalLanguage", "currentOverviewJson", "allowedValuesJson"],
    optionalVariables: [],
  },
  promo_template_recommender: {
    ...
    body: [
      ...
      "Prefer purpose, audience, tone, market, CTA support, and content-density fit.",
