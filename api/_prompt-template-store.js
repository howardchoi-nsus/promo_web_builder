...
const PROMPT_TYPES = {
  ...
  section_composition_planner: {
    ...
    body: [
      ...
      "CTA labels must be concise, action-oriented, use 2 to 4 words where appropriate, and never exceed 20 Unicode characters including spaces.",
  ...
  promo_overview_parser: {
    ...
    body: [
      ...
      "Generate a useful title, a concise supporting lead text, a short action-oriented CTA label, classify purpose/audience/tone, and propose a qualitative main offer when safe.",
      "CTA label must be concise, impactful, use 2 to 4 words where appropriate, and never exceed 20 Unicode characters including spaces.",
  ...
  promo_template_composer: {
    ...
    body: [
      ...
      "Map each compatible selected component exactly once. Prefer title to title/headline, leadText to lead/eyebrow/subtitle, mainOffer to description/copy/body, and ctaLabel only to CTA fields.",
      "Never bind title, leadText, or mainOffer to a CTA field. CTA labels must not exceed 20 Unicode characters including spaces.",
  ...
  promo_page_composer: {
    ...
    body: [
      "Compose one promotion page from the confirmed overview and the server-approved candidate snapshot.",
      "Use only supplied IDs, enum values, content paths, layout variants, motion presets, and design token versions.",
      "Include every required section exactly once and never duplicate a section unless its policy explicitly allows it.",
      "Never return HTML, CSS, JavaScript, selectors, URLs, raw coordinates, or invented IDs.",
      "Preserve locked content and fixed-position sections.",
      "Bind CTA fields only to a non-empty ctaLabel. Leave a CTA unbound when ctaLabel is empty. CTA labels must be concise, action-oriented, and no longer than 20 Unicode characters including spaces.",
      "Confirmed overview: {{overviewJson}}",
      "Approved candidates and policies: {{candidateSnapshotJson}}",
      "Execution constraints: {{constraintsJson}}",
    ].join("\n"),
  ...
  promo_composition_editor: {
    ...
    body: [
      "Convert the user's requested change into only the allowlisted composition operations.",
      "Use only target instance IDs, field paths, token IDs, layout variants, and motion preset IDs present in the current snapshot.",
      "Never return HTML, CSS, JavaScript, selectors, raw coordinates, URLs, or invented IDs.",
      "Do not modify locked content or required visibility.",
      "User instruction: {{instruction}}",
      "Current composition: {{currentSnapshotJson}}",
      "Allowed operations: {{allowedOperationsJson}}",
    ].join("\n"),
